...
const DetailFacility = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/facility/${id}`, {
                    headers: {
                        ...
                        loading ? (
                            ...
                            data ? (
                                <>
                                    ...
                                    <Link to="/facility" className="btn btn-outline mt-5">
                                        <IoBackspaceOutline size={25} />Go Back
                                    </Link>
                                </>
                            ) : (
                                <div className="col-span-3">
                                    <Error title="Oops!" desc={`There's something wrong`} link="/facility" />
                                </div>
                            )
                        )
                    }
