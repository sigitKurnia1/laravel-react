...
import { CiClock2, CiCirclePlus } from "react-icons/ci"
...
const UpdateFoodCafe = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/foodcafe/${id}`, {
                    headers: {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            await axios.post(`http://127.0.0.1:8000/api/foodcafe/${id}`, data, {
                headers: {
                    "Authorization": `Bearer ${token}`,
                ...
                <div className="grid grid-cols-1 justify-items-center">
                    {
                        loading ? (
                            ...
                            <div className="col-span-3">
                                <Error title="Oops!" desc={`There's something wrong`} link="/foodcafe" />
                            </div>
                        ) : (
                            <div className="card w-[500px] bg-base-100 shadow-xl">
                            <div className="card-body">
                                    <p className="text-center text-xl font-semibold">Update FoodCafe</p>
                                    <form className="mt-5" onSubmit={handleUpdate}>
                                        <label className="input input-bordered flex items-center gap-2 mb-5">
                                            <LuHotel className="text-slate-300" size={25} />
                                            <input name="name" type="text" className="grow" placeholder="Hotel FoodCafe" onChange={handleChange} value={formData.name} />
                                        </label>
                                        <label className="input input-bordered flex items-center gap-2 mb-5">
                                            <RiPinDistanceLine className="text-slate-300" size={25} />
                                            <input name="distance" type="number" className="grow" placeholder="Hotel FoodCafe" onChange={handleChange} value={formData.distance} />
                                        </label>
                                        ...
                                        <textarea name="narration" className="textarea textarea-bordered mb-3.5 w-full" placeholder="FoodCafe Narration" cols={66} rows={3} onChange={handleChange} value={formData.narration}></textarea>
