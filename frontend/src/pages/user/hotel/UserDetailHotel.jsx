...
const UserDetailHotel = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/user-hotel-detail/${id}`, {
                    headers: {
                        ...
                        loading ? (
                            ...
                            data ? (
                                <>
                                    ...
                                    <Link to="/user-hotel" className="btn btn-outline mt-5">
                                        <IoBackspaceOutline size={25} />Go Back
                                    </Link>
                                </>
                            ) : (
                                <div className="col-span-3">
                                    <Error title="Oops!" desc={`There's something wrong`} link="/user-hotel" />
                                </div>
                            )
                        )
                    }
